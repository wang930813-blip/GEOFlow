...
| `pubType` | number | **是** | - | 发布类型，必传：1-立即发布，0-存草稿 |
...
| `erchuang` | number | **是** | - | 允许转载，必传：0-不允许，1-允许 |
| `danmu` | number | **是** | - | 弹幕，必传：0-关闭，1-开启 |
| `comments` | number | **是** | - | 评论，必传：0-关闭，1-开启 |
| `curated_comments` | number | **是** | - | 精选评论，必传：0-关闭，1-开启 |
| `mentions` | array | 否 | `[]` | @好友列表（≤10个） |
| `self_captions` | number | **是** | - | 投稿字幕，必传：0-不投，1-投稿 |
| `dolby_voice` | number | **是** | - | 杜比音效，必传：0-关闭，1-开启 |
| `fans_desc` | string | **是** | - | 粉丝动态描述，必传（≤200字，可传空串） |
...
| `taskId` | string | **是** | - | 任务ID |
| `pubType` | number | **是** | - | 发布类型，必传：1-立即发布，0-存草稿 |
| `original` | number | **是** | - | 原创标识，必传：1-原创，0-转载 |
| `timing` | number | **是** | - | 定时时间戳，必传，0-立即发布 |
