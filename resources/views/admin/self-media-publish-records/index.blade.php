...
@section('content')
    @php
        ...
        $platformLogo = fn (string $platform): string => \App\Support\SelfMedia\SelfMediaPlatformCatalog::logoPath($platform);
        ...
        $siteDisplayName = (string) ($scopeLabel ?? ($site instanceof \App\Models\Site ? $site->name : '代理下属用户'));
    @endphp
    ...
    <div class="space-y-5">
        <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
            <div>
                ...
                <p class="mt-1 text-sm text-gray-600">查看平台任务状态和发布链接。</p>
            </div>
                    ...
                    <tbody class="divide-y divide-slate-200 bg-white">
                        @forelse($records as $record)
                            @php
                                ...
                                $accountName = trim((string) ($account?->account_name ?? '')) ?: (string) ($account?->external_account_id ?? $record->external_account_id ?? '-');
                            ...
                            <tr>
                                <td class="px-5 py-4 align-top">
                                    ...
                                    @if((string) ($job?->external_flow_id ?? '') !== '')
                                        <div class="mt-1 text-xs text-slate-400">Flow：{{ $job->external_flow_id }}</div>
                                    @endif
                                </td>
                                <td class="px-5 py-4 align-top">
