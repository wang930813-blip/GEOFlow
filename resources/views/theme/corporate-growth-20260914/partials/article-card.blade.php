...
<article class="cg-article-card">
    <a href="{{ route('site.article', $article->slug) }}" class="cg-article-card__visual{{ $coverImage !== '' ? ' has-image' : '' }}" aria-hidden="true">
        @if($coverImage !== '')
    ...
    <div class="cg-article-card__body">
        ...
        @if($summary !== '')
            <p>{{ \Illuminate\Support\Str::limit($summary, 128) }}</p>
        @endif
        <a href="{{ route('site.article', $article->slug) }}" class="cg-link">
