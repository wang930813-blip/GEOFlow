...
- 配套 Skill 定位为“策影GEO品牌增长智能体”，以品牌增长为核心，覆盖品牌建设和推广执行；品牌诊断只是其中一项检测能力。Skill 自然识别品牌或产品推广、竞争分析、品牌定位、AI 搜索可见性、内容资产、媒体传播和效果评估需求，不依赖用户显式输入 GEO 或 MCP。
...
- 自然触发场景：品牌增长目标下的品牌建设、推广执行、广告软文、推广软文、宣传稿、营销文章、发布到站点或官网、视频生成和结果查询、文章站内发布、媒体投稿和进度查询、素材管理、任务执行、竞争对手分析、品牌定位评估、AI 搜索可见性、内容与媒体增长、推广效果评估。
...
安装时保留压缩包中的完整根目录，将其放入客户端声明的 Agent Skills 目录。安装后可直接询问“写一篇某产品广告软文发布到站点”“写一篇某项目宣传文章发到官网”“我应该如何推广品牌”“我的竞争对手是谁”“品牌定位是否有竞争力”等自然业务问题，不需要在输入中加入 GEO、ceying-geo 或 MCP。Skill 在首次需要平台数据时检查 `geo_*` 工具：支持依赖安装的客户端优先打开 `ceying-geo` 连接流程，其他客户端按实际产品界面提供手动配置引导。客户端不支持 Agent Skills 时，可复制管理页提供的兼容指令继续使用 MCP 工具。
...
Skill 采用“自然语言操作台”方式工作。用户明确要求生成视频、查询视频结果、保存文章、发布到站点、媒体投稿、查询投稿进度、管理素材、执行任务或查看结果时，直接匹配对应 `geo_*` 工具，不强制先走诊断、策略、内容、发布的顺序。用户只提出开放式推广、定位、竞争或曝光问题时，再输出增长方案并列出可执行动作。
...
3. 生成成功必须返回视频任务编号、状态、进度、`first_video_url`、`videos`、`combined_videos` 和封面图字段，不能只回复“任务已创建”。
4. 生成失败时读取 `failure_reason`，不要重复创建同一任务；需要重试时使用新的明确主题或用户确认后的新幂等键。
