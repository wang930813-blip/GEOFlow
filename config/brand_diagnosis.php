...
return [
    ...
    'public_platforms' => [
        ...
        'claude' => [
            'enabled' => filter_var(env('BRAND_DIAGNOSIS_CLAUDE_ENABLED', false), FILTER_VALIDATE_BOOLEAN),
            'label' => 'Claude',
            'icon' => 'CL',
            'logo_path' => '',
            'chat_url' => 'https://claude.ai/',
            'official_share_domains' => ['claude.ai', 'anthropic.com'],
            'base_url' => rtrim((string) env('BRAND_DIAGNOSIS_CLAUDE_BASE_URL', 'https://api.anthropic.com'), '/'),
            'api_key' => (string) env('BRAND_DIAGNOSIS_CLAUDE_API_KEY', ''),
            'model' => (string) env('BRAND_DIAGNOSIS_CLAUDE_MODEL', 'claude-sonnet-4-5-20250929'),
            'timeout' => (int) env('BRAND_DIAGNOSIS_CLAUDE_TIMEOUT', 60),
            'connect_timeout' => (int) env('BRAND_DIAGNOSIS_CLAUDE_CONNECT_TIMEOUT', 10),
            'max_keywords' => (int) env('BRAND_DIAGNOSIS_CLAUDE_WEB_SEARCH_MAX_KEYWORDS', 5),
            'max_tokens' => (int) env('BRAND_DIAGNOSIS_CLAUDE_MAX_TOKENS', 4096),
            'supports_web_search' => filter_var(env('BRAND_DIAGNOSIS_CLAUDE_SUPPORTS_WEB_SEARCH', true), FILTER_VALIDATE_BOOLEAN),
            'anthropic_version' => (string) env('BRAND_DIAGNOSIS_CLAUDE_ANTHROPIC_VERSION', '2023-06-01'),
            'request_style' => 'messages',
        ],
    ],
