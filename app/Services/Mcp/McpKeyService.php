...
class McpKeyService
{
    ...
    public function toolCatalog(): array
    {
        return [
            ...
            ['name' => 'geo_submit_article_to_media', 'scope' => 'media:read + media:submit', 'description' => '将当前账号已有文章投递到指定媒体渠道，提交前检查已有订单避免重复投稿', 'billing' => '按渠道实际售价扣费，失败退款'],
            ['name' => 'geo_publish_article_to_media', 'scope' => 'articles:write + media:read + media:submit', 'description' => '保存 AI 文章并立即投递到指定媒体渠道，提交前检查已有订单避免重复投稿', 'billing' => '按渠道实际售价扣费，失败退款'],
