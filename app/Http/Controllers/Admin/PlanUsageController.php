...
class PlanUsageController extends Controller
{
     ...
     */
    private function buildUsageRows(
    ...
    ): Collection
    {
        ...
        return $subscriptions->map(function (AdminPlanSubscription $subscription) use ($resourceCatalog, $usageBySubscription, $creditAccounts, $mediaDeliveryUsage, $deliveryStats, $manualUsageBySite): array {
            ...
            $resources = collect((array) $subscription->entitlements_snapshot)
                ...
                ->map(function (array $entitlement, string $key) use ($resourceCatalog, $usages, $manualUsageByResource, $creditAccount): array {
                    ...
                    $used = array_key_exists($key, $manualUsageByResource)
                        ...
                        : (int) ($usages->get($key)?->used_amount ?? 0);
