...
class AdminWelcomeIntroCopyTest extends TestCase
{
    public function test_intro_copy_is_updated_for_version_2_first_deployment_guide(): void
    {
        ...
        $this->assertSame('GEO 2.0 首次部署说明', $copy['zh-CN']['letter']['title']);
        $this->assertSame('GEO 2.0 First Deployment Guide', $copy['en']['letter']['title']);
        $this->assertStringNotContainsString('策影', json_encode($copy, JSON_UNESCAPED_UNICODE | JSON_THROW_ON_ERROR));
